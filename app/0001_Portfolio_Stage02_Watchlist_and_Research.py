...
tab1, tab2, tab3, tab4, tab5 = st.tabs(["📁 Upload CSV", "📊 Portfolio View", "📈 SGB vs Gold","Performance","Candle stick chart"])
...
with tab4:
    st.header("📉 Performance (Holdings vs NIFTY 50)")

    portfolio_df = load_portfolio()
    if portfolio_df.empty:
        st.info("Upload portfolio first (Tab 1).")
    else:
        # Only equities/ETFs for now; skip SGB (because you already treat it as gold proxy) [file:1]
        ...
        if not tickers:
            ...
            symbols = [to_yf_symbol(t) for t in tickers] + [NIFTY50_YF]

            close = fetch_close_matrix(symbols, period=period)
            if close.empty:
                st.warning("No data returned from yfinance for the selected period.")
            else:
                rr = relative_return(close)

                st.subheader("📈 Relative return (Base = 100)")
                fig = px.line(rr, x=rr.index, y=rr.columns)
                fig.update_layout(legend_title_text="Symbol", height=550)
                st.plotly_chart(fig, use_container_width=True)

                st.subheader("📊 Snapshot (end vs start)")
                last = rr.iloc[-1].sort_values(ascending=False).rename("Return_Index")
    ...
    if portfolio_df.empty:
        ...
        if not tickers:
            ...
            if hist is None or hist.empty or hist["Close"].isna().all():
                ...
                # market profile bins (same concept as your OldApp_2) [file:3]
                bins = st.slider("Profile bins", 10, 60, 20, 5)
                pmin, pmax = float(prices.min()), float(prices.max())
                price_ax = np.linspace(pmin, pmax, num=bins)
                vol_ax = np.zeros(bins)

                # bucket volume by last-trade close into bins
                # simple binning; consistent with the “if in range then add volume” style you had [file:3]
                ...
                fig.add_trace(
                    go.Bar(x=vol_ax, y=price_ax, orientation="h"),
                    row=1, col=1
                )
                ...
                fig.update_layout(height=750, showlegend=False, title=f"{symbol} — Candles + Profile")
